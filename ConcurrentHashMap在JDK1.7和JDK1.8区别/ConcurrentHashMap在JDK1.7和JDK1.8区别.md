...
​	 Segment 数组长度在初始化时需要确定，默认为16，后续不再改变，在运行时无法动态调整，这是分段锁实现核心设计之一。
...
​	 jdk1.7和jdk1.8ConcurrentHashMap大量使用CAS操作来减少锁竞争，实现高效并发操作，例如哈希表初始化、新节点插入空桶、自旋机制等；
...
4. 链表迁移：和jdk1.7版本类似，有**节点优化机制**，桶迁移完成时设置`ForwardingNode`，同时代表哈希表正在迁移，这时如遇写操作可以协助扩容，读操作可以跳转到新桶进行查询；
5. 红黑树迁移：类似链表；
...
​	当扩容过程中遇到写数据会触发协助扩容机制，当完成所有桶迁移完成后继续写数据逻辑，例如在putVal方法触发：

```java
final V putVal(K key, V value, boolean onlyIfAbsent) {
	// ...代码省略
	// 死循环，如遇协助扩容，协助扩容完成后tab为新表，继续写入数据逻辑
